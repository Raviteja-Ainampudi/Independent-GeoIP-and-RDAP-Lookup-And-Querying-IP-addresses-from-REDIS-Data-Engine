...
def geoquery(db, lis):    
    """This function is used to filter the available 12 parameters of data from the parsed lookups for GeoIP. All 12 paramters are strings. It returns a list of desired IP address which satisfy all the filter parameters.
    ...
    for i, j in db.items():
        count = 0
        for k in lis:
            if str(j[trigger.index(k)]) == str(lis[k]):
                count +=1
...
def rdapquery(db, lis):
    ...
    for i,j in db.items():
        ...
        for k in lis:
            if k in trigger1:
                if str(j[trigger1.index(k)]) == str(lis[k]):
                    count +=1
            if k in trigger2:
                if str(lis[k]) in j[trigger2.index(k)+5]:
                    count+=1
        if count == len(list(lis)):
            filter_data.append(i)
    if len(filter_data)>0:
